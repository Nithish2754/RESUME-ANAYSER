...
class ResumeParser(object):
    ...
    def __get_basic_details(self):
        ...
        try:
            ...
            name_found = False
            for line in self.__text_raw.split('\n'):
                clean_line = ''.join(c for c in line if c.isalpha() or c.isspace()).strip()
                if clean_line and 1 <= len(clean_line.split()) <= 4 and clean_line.lower() not in ['resume', 'curriculum vitae', 'cv', 'profile']:
                    self.__details['name'] = clean_line.title()
                    name_found = True
                    break
            if not name_found:
                person_ents = [ent.text for ent in self.__nlp.ents if ent.label_ == 'PERSON']
                if person_ents:
                    self.__details['name'] = person_ents[0]
                else:
        ...
        try:
            ...
            pass
        if not self.__details.get('degree'):
            try:
