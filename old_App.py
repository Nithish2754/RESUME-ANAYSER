...
def course_recommender(course_list):
    st.subheader("**Courses & Certificates Recommendations ≡ƒæ¿ΓÇì≡ƒÄô**")
    c = 0
    rec_course = []
...
def run():
    ...
    link = '<b>Built with ≡ƒñì by <a href="https://dnoobnerd.netlify.app/" style="text-decoration: none; color: #021659;">Deepak Padhi</a></b>' 
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                st.header("**Resume Analysis ≡ƒñÿ**")
                st.success("Hello "+ resume_data['name'])
                st.subheader("**Your Basic info ≡ƒæÇ**")
                try:
                ...
                st.subheader("**Skills Recommendation ≡ƒÆí**")
                ...
                for i in resume_data['skills']:
                    ...
                    if i.lower() in ds_keyword:
                        ...
                        st.markdown('''<h5 style='text-align: left; color: #1ed760;'>Adding this skills to resume will boost≡ƒÜÇ the chances of getting a Job</h5>''',unsafe_allow_html=True)
                    ...
                    #### Web development recommendation
                        ...
                        st.markdown('''<h5 style='text-align: left; color: #1ed760;'>Adding this skills to resume will boost≡ƒÜÇ the chances of getting a Job≡ƒÆ╝</h5>''',unsafe_allow_html=True)
                        # course recommendation
                ...
                st.header("**Bonus Video for Resume Writing Tips≡ƒÆí**")
                resume_vid = random.choice(resume_videos)
                st.video(resume_vid)

                ## Recommending Interview Preparation Video
                st.header("**Bonus Video for Interview Tips≡ƒÆí**")
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                fig = px.pie(values=values, names=labels, title="Chart of User Rating Score From 1 - 5 ≡ƒñù", color_discrete_sequence=px.colors.sequential.Aggrnyl)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='Predicted Field according to the Skills ≡ƒæ╜', color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
                st.plotly_chart(fig)
                ...
                # Pie chart for User's≡ƒæ¿ΓÇì≡ƒÆ╗ Experienced Level
                st.subheader("**Pie-Chart for User's Experienced Level**")
                fig = px.pie(df, values=values, names=labels, title="Pie-Chart ≡ƒôê for User's ≡ƒæ¿ΓÇì≡ƒÆ╗ Experienced Level", color_discrete_sequence=px.colors.sequential.RdBu)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='From 1 to 100 ≡ƒÆ»', color_discrete_sequence=px.colors.sequential.Agsunset)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='Usage Based On IP Address ≡ƒæÑ', color_discrete_sequence=px.colors.sequential.matter_r)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='Usage Based On City ≡ƒîå', color_discrete_sequence=px.colors.sequential.Jet)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='Usage Based on State ≡ƒÜë', color_discrete_sequence=px.colors.sequential.PuBu_r)
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values=values, names=labels, title='Usage Based on Country ≡ƒîÅ', color_discrete_sequence=px.colors.sequential.Purpor_r)
                st.plotly_chart(fig)
